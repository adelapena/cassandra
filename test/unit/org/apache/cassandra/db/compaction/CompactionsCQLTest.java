...
//    }
    ...
    public void testLCSThresholdParams() throws Throwable
    {
        ...
        assertEquals(String.format("Expected:<50> but was:<%d>: %s", sstables.size(), sstables.stream().map(SSTable::toString).collect(Collectors.joining(","))),
                     50, sstables.size());
        ...
        sstables = act.transaction.originals();
        assertEquals(String.format("Expected:<50> but was:<%d>: %s", sstables.size(), sstables.stream().map(SSTable::toString).collect(Collectors.joining(","))),
                     50, sstables.size());
