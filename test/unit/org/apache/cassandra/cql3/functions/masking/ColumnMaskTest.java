...
public class ColumnMaskTest extends ColumnMaskTester
{
    ...
    public void testViews() throws Throwable
    {
        ...
        assertRowsNet(format("SELECT v FROM %s.%s", KEYSPACE, view), row("redacted"));
        assertRowsNet(format("SELECT v FROM %s.%s WHERE v='sensitive'", KEYSPACE, view), row("redacted"));
        assertRowsNet(format("SELECT v FROM %s.%s WHERE v='redacted'", KEYSPACE, view));

        alterTable("ALTER TABLE %s ALTER v WITHOUT MASK");
        assertRowsNet(format("SELECT v FROM %s.%s", KEYSPACE, view), row("sensitive"));
        assertRowsNet(format("SELECT v FROM %s.%s WHERE v='sensitive'", KEYSPACE, view), row("sensitive"));
        assertRowsNet(format("SELECT v FROM %s.%s WHERE v='redacted'", KEYSPACE, view));
    }
    ...
    public void testGroupBy() throws Throwable
    {
        ...
        assertRowsNet(query, row(1, 0, "sensitive"), row(0, 0, "sensitive"));

        // with masked regular column
        alterTable("ALTER TABLE %s ALTER v MASKED WITH mask_replace('redacted')");
        assertRowsNet(query, row(1, 0, "redacted"), row(0, 0, "redacted"));

        // with masked clustering key
        alterTable("ALTER TABLE %s ALTER c MASKED WITH mask_replace(-1)");
        assertRowsNet(query, row(1, -1, "redacted"), row(0, -1, "redacted"));

        // with masked partition key
        alterTable("ALTER TABLE %s ALTER k MASKED WITH mask_replace(-1)");
        assertRowsNet(query, row(-1, -1, "redacted"), row(-1, -1, "redacted"));
