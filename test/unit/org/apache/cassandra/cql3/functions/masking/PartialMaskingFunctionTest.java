...
public class PartialMaskingFunctionTest extends MaskingFunctionTester
{
    ...
    {
        String valueAsText = valueAsText(type, value);
        ...
        for (PartialMaskingFunction.Type masker : PartialMaskingFunction.Type.values())
        {
            ...
            assertRows(execute(format("SELECT %s(%s, 1, 2) FROM %%s", functionName, name)),
                       row(masker.mask(valueAsText, 1, 2, PartialMaskingFunction.DEFAULT_PADDING_CHAR)));

            // ... with manually specified ASCII padding
            assertRows(execute(format("SELECT %s(%s, 1, 2, '#') FROM %%s", functionName, name)),
                       row(masker.mask(valueAsText, 1, 2, '#')));

            // ... with manually specified UTF-8 padding
            assertRows(execute(format("SELECT %s(%s, 1, 2, 'é') FROM %%s", functionName, name)),
                       row(masker.mask(valueAsText, 1, 2, 'é')));
            ...
            assertRows(execute(format("SELECT %s(%s, null, 2) FROM %%s", functionName, name)),
                       row(masker.mask(valueAsText, 0, 2, PartialMaskingFunction.DEFAULT_PADDING_CHAR)));
    ...
    private <T> String valueAsText(CQL3Type cqlType, T value)
    {
        if (isNullOrEmptyMultiCell(cqlType, value))
            return null;

        AbstractType<T> type = (AbstractType<T>) cqlType.getType();

        if (type.isTuple() || type.isUDT())
            return type.toCQLString(makeByteBuffer(value, type));

        return type.getSerializer().toString(value);
    }
