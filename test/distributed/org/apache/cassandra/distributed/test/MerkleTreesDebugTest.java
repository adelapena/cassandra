...
public class MerkleTreesDebugTest extends TestBaseImpl
{
    ...
    private static final int CLUSTERINGS = 100;

    @Test
    public void Test() throws Throwable
    {
        try (Cluster cluster = init(builder().withRacks(1, 2, 4)
...
//            NodeToolResult res = cluster.get(1).nodetoolResult("repair", KEYSPACE);
            NodeToolResult res = cluster.get(1).nodetoolResult("repair", "-j=3", KEYSPACE);
//            NodeToolResult res = cluster.get(1).nodetoolResult("repair", "-seq", KEYSPACE);
            res.asserts().success();
        }
    }
}
