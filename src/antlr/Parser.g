...
alterTableStatement returns [AlterTableStatement.Raw stmt]
    ...
    : K_ALTER K_COLUMNFAMILY (K_IF K_EXISTS { ifExists = true; } )?
      ...
      | K_ALTER ( K_IF K_EXISTS { $stmt.ifColumnExists(true); } )? id=cident mask=columnMask { $stmt.mask(id, mask); }

      | K_ALTER ( K_IF K_EXISTS { $stmt.ifColumnExists(true); } )? id=cident K_WITHOUT K_MASK { $stmt.unmask(id); }
