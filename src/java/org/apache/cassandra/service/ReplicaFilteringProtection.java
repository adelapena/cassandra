...
{
    /** The aimed max number of replica rows that are cached when we haven't detected that we have rows to fetch. */
    private static final int CACHED_ROWS_PER_PARTITION_WITHOUT_CONFLICTS = 100;

    /** The aimed max number of replica rows that are cached when we have detected that we have rows to fetch. */
    private static final int CACHED_ROWS_PER_PARTITION_WITH_CONFLICTS = 1000;
            ...
            return new CachedRowIterator()
            {
                ...
                private UnfilteredRowIterator fetch()
                {
                    ...
                    while (unprotectedPartition != null && unprotectedPartition.hasNext() &&
                           (toFetch == null && cachedRows.size() < CACHED_ROWS_PER_PARTITION_WITHOUT_CONFLICTS
                            || toFetch != null && cachedRows.size() < CACHED_ROWS_PER_PARTITION_WITH_CONFLICTS))
                    {
