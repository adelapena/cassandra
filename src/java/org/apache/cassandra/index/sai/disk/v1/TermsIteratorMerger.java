...
{
    private final List<TermsIterator> iterators;
    ...
    public TermsIteratorMerger(List<TermsIterator> iterators, AbstractType<?> type)
    {
        this.iterators = iterators;
        this.mergedIterator = new MergingIterator(iterators);
        this.type = type;
    }
    ...
    {
        PriorityQueue<PostingList.PeekablePostingList> postingLists = new PriorityQueue<>(100, Comparator.comparingLong(PostingList.PeekablePostingList::peek));
        for (int x = 0; x < mergedIterator.getNumTop(); x++)
        {
            int index = mergedIterator.top[x].index;
            TermsIterator termsIterator = iterators.get(index);
