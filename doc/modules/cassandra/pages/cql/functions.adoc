...
| `mask_replace(value, replacement])` | Replaces the first argument by the replacement value on the second argument. The replacement value doesn't need to have the same type as the replaced value.
...
`mask_replace(123, 'REDACTED')` -> `'REDACTED'`
...
`mask_inner(12345, 1, 2)` -> `'1**45'`

`mask_inner([1, 2, 3], 2, 1)` -> `'[1******]'`
...
`mask_outer(12345, 1, 2)` -> `'*23**'`

`mask_outer([1, 2, 3], 1, 1)` -> `'*1, 2, 3*'`
